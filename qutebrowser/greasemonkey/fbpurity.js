...
//
// (C) 2009 - 2026 Stephen Fernandez - https://www.fbpurity.com
//
...
// 37.2.0   3rdMar2025  Added News Sort: Friends Feed option that permanently redirects the Newsfeed to the Friends Feed page, Fixed a bug in the show deactivated friends function
// 37.3.3  26thMar2025  Fixed Hide Sponsored Posts option, Fixed Hide Reels option, Added News Sort: Favorites option, Fixed(ish) the Hide Stories X button, Added soft navigation detection, Fixed and Updated Hide All Videos option to catch Reels, Removed unread notifications background colour option, Removed sort notifications option, Removed Hide Rooms option, Removed Expand News Column option, Removed obsolete Friend List options, Removed obsolete chat options
// 37.4.0  25thMar2025  Partial fix for newsfeed filtering (should hide most sponsored posts now, and most filtering should work again) Fixed hover shadow for "Show more friends in chat" option
// 37.4.1  22ndApr2025  Added option to hide Meta AI link in left column options
// 37.4.2   7thJun2025  Fixed the Hide "Pages", "Reels", "Groups", "Videos" left column link options, Added Hide Friends left column link option, Fixed the "Clear Ad Topics" button on ad preferences page
// 37.5.0  22ndApr2025  Fixed hiding Sponsored Posts
// 37.5.1  27thJun2025  Added a hide Meta AI link in "Chat options" that hides the  Meta AI link at the top of the contacts list, Fixed the Clear "Ad Topics" functionality in Facebook's Ad Preferences section
// 37.5.5   3rdJul2025  Fixed hiding sponsored posts for languages other than English in certain circumstances (e.g when switched to a Page Profile), disabled background colour option because it was only causing problems (hidden images etc), made hide sponsored marketplace ads functionality work for more languages
// 37.5.6   5thJul2025  Fixed bug in hiderightcolumnsection that was hiding the contacts list in the right column if hide sponsored box / posts option was enabled, Disabled "R" key chronosort function as its obsolete.
// 37.5.7   8thJul2025  Fixed bug in hide sponsored posts in marketplace that was hiding the users own marketplace listings
// 37.5.9  14thJul2025  Fixed bug in newsfeed filtering that meant occasionally posts would slip past the filter, disabled "Enable FBP Video Player" option as its not working properly anymore, whitelisted posts from fbp fan page, experimental auto click skip button on video ads
// 37.5.10 22ndJul2025  Fixed bug where first post in feed was sometimes hidden as a sponsored post, when it actually wasnt one
// 38.0.2  21stAug2025  New Hide Suggested Posts option (separated the functionality out from the hide sponsored posts option), Fixed the Hide Auto Tag Suggestions option, Fixed and renamed the Friends Whitelist feature, its now called "Friend Allow List", Fixed Clear Ad Prefs / Interests button on https://www.facebook.com/adpreferences/?section=interests page, Fixed removing Text Shadow effect for "Normalize Big Font Status" option, Fixed hide Reels option, Removed Hide Profile Hovercards option, Fixed FBP News notifications function, Improved some of FBP's interface elements in Dark Mode
// 38.0.4   4thSep2025  Fixed multiple "X" hide buttons being added to the Stories box, Removed the Hide "All Page Posts" option as it no longer works, Moved friend whitelist code after the hide sponsored box code so ads wouldnt show if the whitelisted friend had liked the Page that was displaying the ad, new option to hide Chat with AIs left column link, Fixed incorrect hiding of marketplace ads shared via messenger/chat
// 38.1.1  11thSep2025  Added Comment Sort : All Comments and Comment Sort : Newest options to let you choose which one you want the default comment sorting mode to be
// 38.1.2  18thSep2025  Fixed hiding sponsored box at bottom right of individual marketplace listings
// 38.1.3  30thSep2025  Renamed hide watch button (top navigation bar option) to hide reels button. Removed hide Quick Help button in top navbar options (button no longer exists)
// 38.1.4   4thOct2025  Added Suggested posts from a group to hide Suggested Posts option
// 38.2.0  11thOct2025  Fixed Newsfeed Filtering, Renamed "News Sort : Most Recent" option to "News Sort : Feeds (Most Recent)"
// 38.3.0  15thNov2025  Added extra Sponsored Post hiding code, Fixed silly undefined variable bug
// 38.4.0  15thJan2026  Fixed Newsfeed filtering functionality
...
(function() {
  ...
  const FBPFanPageURL = 'fluffbustingpurity';
  const FBPFanPageURL2= 'bookfacepurity';
  var FBPFanPageURLRegex = new RegExp("facebook.com\/(" + FBPFanPageURL + "|"  +  FBPFanPageURL2 + ")");
  const FBPFanPageID="408502197558"; //158385957596495
  const FBPFanPageName="Fluff Busting \( FB \) Purity";
  const FBPDevPageID="101923308327295"; //(changed since facebook deleted the fbp testing page,   fb.com/revertsite ) (was 464154953615751)
  const FBPDevPageName="Fluff Busting Purity Development & Testing";
  const FBPfanpagesmallproficon="data:image/png;base64,iVBORw0KGgoAAAANSUhEUgAAAAEAAAABCAQAAAC1HAwCAAAAC0lEQVR42mP8/wQAAwAB/JsCYwAAAABJRU5ErkJggg==";//"https://i.imgur.com/lHHVfYU.png";//https://i.imgur.com/m3ezkAp.png
  ...
  var fbpStyleNormal= ".fbpblocked {display:none !important} .fbpblockedx,.fbpblockedx.fbpnormal {display:none !important} .fbpnormal {display:block !important} div.fbpWL {border: 2px solid purple} div.fbpWL:after {color:green !important;position:absolute;right:10px;top:5px;font-size:13px;font-weight:normal;content:'FBP Allow List'} ";
  if(debug)
  ...
  var langCode, adString, adStringLength, adStringRegEx, sponsormatchRX, adString2; // adString2 = secondary adstring ("Advertiser link" in EN for example)
  // set a placecholder adString (because of timing issues, sometimes adString is not set before certain code runs...
  adString="Sponsored";
  ...
  const individualpostdomquery="div[aria-posinset]";
...
'<br/>&nbsp;<b>FBP Tip</b>: Want to help test out new versions of FBP before they are officially released? <a href="https://www.facebook.com/revertsite">Join the FBP Beta Testing Page</a>.',
...
'<br/>&nbsp;<b>FBP Tip</b>: You can Follow <a target="_blank" href="https://twitter.com/fbpurity">FB Purity News on Twitter</a>.',
//You can get <a target="_blank" href="https://feedburner.google.com/fb/a/mailverify?uri=FacebookPurity">FB Purity News via Email</a>, 
...
//v.addTableRow('All Page Posts','Hide all posts, posted by "Pages", from your newsfeed','pagestory', 'checkbox'); // commented out because the option no longer works, and is not easy to fix
...
['Friends','lcfriends','Hide the Friends Link in the left column'],
...
['Meta AI','lcmetaai','Hide the Meta AI link in ht left column'],
['Meta Quest','lcoculus',''],
['Moves','lcmoves',''],
...
v.addTableRow('Hide Meta AI','Hide the Meta AI link at the top of the contacts list in the right column','rcmetaai','checkbox');
...
//v.addTableRow('Hide Add Files Button','Hide the Add Files Button in Chat','chataddfilesbutt','checkbox');
v.addTableFooter();
newoptscolumn+=v.buildTable();
...
'<tr><td></td><td><b></b></td></tr><tr title="Hides the Reels button in the top navigation bar"><td><label for="topnavwatch" class="fbpoptslabel">Hide Reels Button</label></td><td><input id="topnavwatch" type="checkbox"></td></tr>' +
...
'<tr><td></td><td><b></b></td></tr><tr title="Hides the Account Switcher button in the top navigation bar"><td><label for="hideaccountswitcher" class="fbpoptslabel">Hide Account Switcher Button</label></td><td><input id="hideaccountswitcher" type="checkbox"></td></tr>' +

'<tr><td></td><td><b></b></td></tr><tr style="background-color:#ECEFF5 !important" title="Freezes the top nav bar so it doesnt disappear when the page is scrolled down"><td><label for="freezetopnav" class="fbpoptslabel">Freeze Top Nav Bar</label></td><td><input id="freezetopnav" type="checkbox"></td></tr>' +
'<tr><td></td><td><b></b></td></tr><tr title="Hides the search box in the top navigation bar"><td><label for="hidesearchbox" class="fbpoptslabel">Hide Search Box</label></td><td><input id="hidesearchbox" type="checkbox"></td></tr>' +
'<tr><td></td><td><b></b></td></tr><tr style="background-color:#ECEFF5 !important" title="Hides the Search box pop up suggestions in the top navigation bar"><td><label for="hidesearchpopup" class="fbpoptslabel">Hide Search Box Popup</label></td><td><input id="hidesearchpopup" type="checkbox"></td></tr>' +
'<tr><td></td><td><b></b></td></tr><tr title="Hides the Trending / Popular / Recent  Search Suggestions in the Search box pop up"><td><label for="hidesearchtrending" class="fbpoptslabel">Hide the Trending Search Suggestions</label></td><td><input id="hidesearchtrending" type="checkbox"></td></tr>' +
'<tr><td></td><td><b></b></td></tr><tr style="background-color:#ECEFF5 !important" title="Hides the Cookies Policy Bar in the Top Navigation Bar"><td><label for="cookiespolicy" class="fbpoptslabel">Hide the "Cookies Policy" bar</label></td><td><input id="cookiespolicy" type="checkbox"></td></tr>' +
...
'<span id="appwhitelistheader"><a title="Click to access the Allow List" href="#">Allow List</a></span> <span>|</span> ' +
'<span id="friendwhitelistheader" title="Click to access the Friend Allow List"><a href="#">Friend Allow List</a></span> | ' +
'<span id="custcssheader"><a title="Click to access the Custom Style Sheet (CSS) Box" href="#">CSS</a></span><br/>' +
'<span id="custextdesc" style="background:#ECEFF5;" title="If FBP finds any of the words or phrases you specify in a post in the Newsfeed, FBP will hide the post. Please note, \'Regular Expressions\' can also be used.">Enter the words or phrases, on separate lines, that you wish to filter from the NewsFeed. (<a title="Help with the Text Filter" target="_blank" href="https://www.fbpurity.com/extras.htm#customtextfilter"><b>Text Filter Help</b></a>) <br/></span><span style="display:none" id="autohidedesc" title="If FBP finds any of the words or phrases you specify in a post in the Newsfeed, FBP will \'double hide\' the post from your feed, even hiding it if you click show extras, unless you then click to specifically see the post. Please note, \'Regular Expressions\' can also be used.">Enter the words or phrases, on separate lines, which if they appear in a post, you wish to automatically hide that post from the NewsFeed</span><span style="display:none" id="custcssdesc">Enter the Custom CSS code you would like applied to facebook.  (<a title="Help with Custom CSS" target="_blank" href="https://www.fbpurity.com/facebook-css.htm"><b>CSS Help and examples</b></a>)</span><span style="display:none" id="friendwhitelistdesc">Add friends whose posts you dont want filtered out by FBP to the Allow List (<a title="Help with Friend Allow List" target="_blank" href="https://www.fbpurity.com/whitelist.htm#friends"><b>Friend Allow List Help</b></a>)</span><span style="display:none" id="appwhitelistdesc">Enter words or phrases, one per line, to ensure posts containing them appear in your Newsfeed without being filtered out by FBP. (<a title="Help with Allow List" target="_blank" href="https://www.fbpurity.com/whitelist.htm"><b>Allow List Help</b></a>)</span>' +
...
'<tr style="background-color:#ECEFF5 !important" title="Hiding the sponsored box also hides Sponsored Posts in the newsfeed, Facebook Offers"><td><label class="fbpoptslabel" for="sponsoredbox">Sponsored Box / Posts</label></td><td><input id="sponsoredbox" type="checkbox"></td></tr>'+
'<tr title="Hide Suggested Posts (Join/Follow etc)"><td><label class="fbpoptslabel" for="suggestedposts">Suggested Posts</label></td><td><input id="suggestedposts" type="checkbox"></td></tr>'+
'<tr style="background-color:#ECEFF5 !important" title="Un-Tick this if you dont want FBP to filter out application and game posts from the newsfeed"><td><label class="fbpoptslabel" for="filterappmessages">Game + App Posts</label></td><td><input id="filterappmessages" type="checkbox"></td></tr>'+
'<tr title="Hide the Stories / Create Story box and also the &quot;Your Story&quot; section below the Status update box"><td><label class="fbpoptslabel" for="rcstories">Stories</label></td><td><input type="checkbox" id="rcstories"></td></tr>'+
'<tr style="background-color:#ECEFF5 !important" title="Hides the People You May Know Boxes on the Newsfeed"><td><label class="fbpoptslabel" for="pymk">People You May Know</label></td><td><input id="pymk" type="checkbox"></td></tr>'+
...
'<tr id="hovercardsoption" style="display:none;background-color:#ECEFF5 !important" title="Hides the info popups that show when you hover over a User\'s or Page\'s name or profile picture"><td><label class="fbpoptslabel" for="hovercards">Profile Hovercards</label></td><td><input id="hovercards" type="checkbox"></td></tr>'+
...
'<tr style="background-color:#ECEFF5 !important" title="Force the Newsfeed to stay on \'Most Recent\'. This option keeps your newsfeed permanently redirected to the Feeds/All page which is the new name for the Most Recent page."><td><label class="fbpoptslabel" for="sortmostrecent">News Sort : Feeds (Most Recent)</label></td><td><input id="sortmostrecent" type="checkbox"></td></tr>'+
'<tr style="" title="Force the Newsfeed to stay on \'Friends Feed\'. The friends feed only shows posts from your friends in chronological order, none from Fan Pages, so it should load quicker if you just want to check what your friends are up to."><td><label class="fbpoptslabel" for="sortfriendfeed">News Sort : Friends Feed</label></td><td><input id="sortfriendfeed" type="checkbox"></td></tr>'+
'<tr style="background-color:#ECEFF5 !important" title="Force the Newsfeed to stay on \'Favorites Feed\'. The Favorites feed only shows posts from your Favorite Pages/People in chronological order, so it should load quicker if you just want to check what your Favorite Pages/People are up to."><td><label class="fbpoptslabel" for="sortfavorites">News Sort : Favorites Feed</label></td><td><input id="sortfavorites" type="checkbox"></td></tr>'+

'<tr title="Set the default comment sorting mode to &quot;All comments&quot;"><td><label class="fbpoptslabel" for="commentsortall">Comment Sort : All Comments</label></td><td><input id="commentsortall" type="checkbox"></td></tr>'+
'<tr style="background-color:#ECEFF5 !important"  title="Set the default comment sorting mode to &quot;Newest&quot;"><td><label class="fbpoptslabel" for="commentsortnewest">Comment Sort : Newest</label></td><td><input id="commentsortnewest" type="checkbox"></td></tr>'+

//'<tr  title="Expands the main news column\'s width and hides the reminders column (including birthdays)"><td><label class="fbpoptslabel" for="expandnewscolumn">Expand News Column</label></td><td><input id="expandnewscolumn" type="checkbox"></td></tr>'+
// next 3 lines are hidden as those options are currently obsolete/not working
...
'<tr style="display:none" title="Enable the FBP Video Player for all videos in the Newsfeed"><td><label class="fbpoptslabel" for="showfbpvidplayerlink">Enable FBP Video Player</label></td><td><input id="showfbpvidplayerlink" type="checkbox"></td></tr>'+
...
'<p>In order to keep up to date with news of any changes that may break F.B. Purity and for news of new versions of F.B Purity that fix any problems or add new functionality, it is extremely important that you click the Follow button for the F.B. Purity Fan page on Facebook (below). By doing this you should receive news of updates directly in your newsfeed, and you will also be able to leave feedback about the extension there too.<p/>'+
...
function initundef() {
  ...
  if(typeof (fbpoptsobj.sortfavorites) =='undefined')
    fbpoptsobj.sortfavorites=0;
...
function fbpgetprefs(opts) {
  ...
  // avoid endless reload when news sort option is turned on and fbp is running on mobile website by detecting mobile website and exiting
  if(document.getElementById('app-body')){ // app-body is the id of the body tag on the fb mobile website
    console.log('Exiting fbpgetprefs to stop reload loop on mobile fb site.');
    // if the mobile version of the site is loaded display message and exit
    var insdiv=document.createElement('span');
    insdiv.id="fbpdoesntworkonmobilesite";
    insdiv.textContent="F.B. Purity doesn't work on the mobile version of the website, please switch your browser to desktop mode then reload the page to get FBP to work";
    document.getElementsByTagName('body')[0].parentNode.insertBefore(insdiv,document.getElementsByTagName('body')[0]);
    return; // hopefully this will exit fbp...
  } 
...
function fbpfirstruncheck(result) {
  ...
  // make sure user is logged in (pageLogo element is there)
...
function fbppopulateprefs() {
  ...
  */
  ...
  if(!langCode.match(/^en/))
    document.getElementById("localeproblem").innerHTML="Please note: Your Facebook language setting is: \"<span style='color:black'>" + langCode + "</span>\". If you want FBP's built in options to work correctly <br/>You should <a target='_blank' href='https://www.facebook.com/settings?tab=language'>change your language setting to English (UK or US)</a>. If you dont want to change language, you can create your own newsfeed filters in your own language with the 'Text Filter' box below.";
  /*
...
//	alert(fbpoptsobj.customtextfilter);
  ...
  if(document.getElementById('appwhitelistta').value.length) {
	...
	// replace actual commas with &#44; (ascii code for comma)
	var tempallowlist=document.getElementById('appwhitelistta').value.replace(/,/g,"&#44;").replace(/[\r\n|\r|\n]/g,"\,").replace(/\,{1,}/g,'\,').replace(/,$/,'').replace(/^,/,'');
	fbpoptsobj.userappwhitelist='';
	var ahfarr=tempallowlist.split(',');
	for(i=0;i<ahfarr.length;i++)
	  if (ahfarr[i].replace(/^ {1,}$/,'')!='')
	    fbpoptsobj.userappwhitelist+=ahfarr[i] + ','; // strip out lines that are just spaces.

    fbpoptsobj.userappwhitelist=fbpoptsobj.userappwhitelist.replace(/,$/,''); // strip trailing comma
...
function fbpoptsobjinit() {
  ...
  fbpoptsobj.lcmetaai=0;
  fbpoptsobj.lcaichats=0;
  ...
  fbpoptsobj.commentsortnewest=0;
  fbpoptsobj.commentsortall=0;
}
...
function getUserFullName() {
  try {
  // Attempt to access the CurrentUserInitialData module via require
    const currentUserData = window.require ? window.require('CurrentUserInitialData') : null;
    if (currentUserData && currentUserData.NAME) { // we can also get the USER_ID from this object
      console.log(currentUserData)
      return currentUserData.NAME;
    }
  } catch (e) {
    console.log('Error accessing CurrentUserInitialData:', e);
   }
  return null;
}
...
// comment this section out for testing offline pages
 if(currentuserid=="") { // not logged in yet? just logged out?
...
usernamewhitelist=FBPDevPageName;
/*
if (!currentuserid.length){
  ...
  var usernamewhitelist=FBPFanPageName;
}
else {
  //var useridwhitelistRX=new RegExp(fbpfanpageid + "|" + fbpdevpageid + "|" + currentuserid);
  var currentusername=getUserFullName();
  if(!currentusername)
	console.log('Couldnt determine users full name');
  else
    console.log("username=",currentusername);
  var useridwhitelist=FBPFanPageID + "|" + FBPDevPageID + "|" + currentuserid;
  var usernamewhitelist=FBPFanPageName + "|" + currentusername;
}
*/
...
var fixsortorder = function() {
  ...
  if(fbpoptsobj.sortfriendfeed) {
	feedRedirectURL="https://www.facebook.com/?filter=friends&sk=h_chr";
	feedRedirectName="Friends Feed";
  }
  if(fbpoptsobj.sortfavorites) {
	feedRedirectURL="https://www.facebook.com/?filter=favorites&sk=h_chr";
	feedRedirectName="Favorites Feed";
  }
  if((fbpoptsobj.sortmostrecent || fbpoptsobj.sortfriendfeed) && !window.location.hash.match(/#topstories/)) { // force feed to sort by most recent //  
	...
	if(window.location.href.match(/\?sk=h_nor$|www\.facebook\.com\/?(?:\?_rdr)?$/)){  //added option ?_rdr at the end
	...
	*/
	...
	// BEGIN trying this code again, seems to be working ok now. (This section of code seems to be related to the old newsfeed design, so can probably go)
	...
	if(homelinksarr && homelinksarr.length>1)
	  for(i=0;i<3;i++){
		if((typeof homelinksarr[i] != "undefined") && (typeof homelinksarr[i].href != "undefined")) {
		  ...
		  if(debug)
			console.log("homelinksarr[", i , "] is undefined");
		}
	  }
},3000);
...
var initstage2 = function () {
  ...
  if(fbpoptsobj.sortfriendfeed && window.location.href.match(/facebook\.com\/?$|\?sk=h_nor|\?sk=h_chr|\?sk=h_nf|\?ref=tn_tnmn|\?ref=logo|\.com\/\?_rdr$/) && (!window.location.href.match(/\?sk=h_nor#topstories/)))
	window.location.href="https://www.facebook.com/?filter=friends&sk=h_chr";

  if(fbpoptsobj.sortfavorites && window.location.href.match(/facebook\.com\/?$|\?sk=h_nor|\?sk=h_chr|\?sk=h_nf|\?ref=tn_tnmn|\?ref=logo|\.com\/\?_rdr$/) && (!window.location.href.match(/\?sk=h_nor#topstories/)))
	window.location.href="https://www.facebook.com/?filter=favorites&sk=h_chr";
...
// live video
  ...
  if(fbpoptsobj.livevideo==1){
	...
	extraTextFilter +=",· This video is live now, is hosting a watch party, is live now\..*·";
  }
  ...
  if((typeof fbpoptsobj.reelsvideo!=='undefined') && (fbpoptsobj.reelsvideo==1)){
    extraTextFilter += ",FacebookReels,Reels  ·,ReelsCreate,FollowReels,Reels  ·,Short Video  ·,Short Video  ·";
	//headerTextFilter += ",shared \\d+ links on YouTube,shared a link on youtube";
  }

  if(typeof fbpoptsobj.suggestedposts=='undefined') // if they have just upgraded, turn on the hide suggestedposts option.
	fbpoptsobj.suggestedposts=1;
  if(fbpoptsobj.suggestedposts==1){ 
    //placeholder (migrate code from line 3642 (code for hiding suggested posts that is mixed with sponsored posts)
	extraTextFilter += ",Recently Released Albums,Pages You May Like,Links you may like,Page Stories you may like,Jobs Near You,^Recommended for you,^Suggested because it's popular on Facebook,X?Events You May Like,suggested shows,Get (every )?New episode(s)? in your watchlist,^Similar to posts, here are groups you might likeC,Suggested for you,Create groupSee all,Suggested posts from a group,FacebookMeta AI";
	headerTextFilter += ",· Follow,· Join";
  }
...
// People You May Know
  if(fbpoptsobj.pymk==1) {
    headerTextFilter += ",^X?People you may know,FacebookPeople you may know,Friend Requests,suggested people to follow,People you may know Profile";
	extraTextFilter += ",People you may knowX,FriendRequests,friend-add,FacebookFriend Requests";
  }

// hide sponsored posts in newsfeed
  if(fbpoptsobj.sponsoredbox==1) {
	//extraTextFilter='Sponsored ·,' + extraTextFilter;
    extraTextFilter += ",Get Offer ·,Advertiser-sponsored poll,· Paid ·,· Paid for by,Paid partnership ·,Paid Partnership  ·,Get offerAll reactions";
	headerTextFilter += ", claimed an offer from, posted an offer,shared an offer,shared a product\.,shared their product\.,posted a job\.";
...
// fix font background colour
  if(fbpoptsobj.fontbgcolourfix==999) { // 999 because the option doesnt work properly any more and only causes problems, so this effectively disables the option
...
// top nav background image
  ...
  if(fbpoptsobj.topnavwatch) {
    fbpfreestyle.textContent +=' div[role="banner"] a[href*="/watch/"],a[href="/watch/?ref=tab"], div[role="banner"] a[href="/reel/?s=tab"] {display:none}';
  }
  ...
  var lcoptsarr="wholeleftcolumn,colouredbg,lcadvertmgr,lcconnect,lcallpages,lcpagesfeed,lclikepages,lcallapps,lcappcenter,lcevents,lcgamesfeed,lcpokes,lcprofile,lcmusic,lcplaceseditor,lcallgroups,lccreategroup,lccreateframe,lccreatepage,lcfindfriends,lcfindnewgroups,lcdeveloper,lcfundraisers,lcfriends,lconthisday,lcbrowse,lcwelcome,lcweather,lcoculus,lccreatorstudio,lclinks,lcoffers,lcmanagegroups,lcsaved,lcshops,lcmarketplace,rcmarketplace,lcsuggested,lcsuggestedits,lccreateevent,lcphotos,lcsalegroups,lcpayments,lcdiscovergroups,lcyourposts,lcmovies,lcmessages,lcmessengerkids,lcrecommendations,lccreate,lcshortcuts,lcinsights,lcjobs,lctownhall,lcorderfood,lcsafetycheck,lcexplorefeed,lcwatch,lcrecentadactivity,lcmoves,lcgreetings,lcgamingvideos,lcvotinginfo,lccommunityhelp,lcnews,lcemohealth,lcbusinessdiscovery,lclivevideos,lccampus,lcblood,lcfavorites,lcadcenter,lcclimatescience,lcreels,lcdigitalcollectibles,lcraybans,wholerightcolumn,rcrecommendedpages,rcrelatedgroups,rcpokes,rcbirthdays,rcfriendrequests,rcgamerequests,rcyourpages,rclive,rcstories,rcwatchlist,rcrelatedvids,rctodaysgames,rcsavedlinks,catphoto,dogphoto,babyphoto,childphoto,memephoto,selfiephoto,couplephoto,foodphoto,drinkphoto,smokingphoto,carphoto,customphotoblocklistfix,imagetagsonhover,typingcomment,seemoreexplore,seemoreshortcuts,hidemesstopnav,hidenotiftopnav,hidecreatetopnav,hideaccountswitcher,hidepagestopnav,topnavwatch,topnavnews,topnavevents,topnavgroups,topnavgaming,topnavmarketplace,lcliftblackvoices,suggestedcomments,lcmetaai,lcaichats,commentsortnewest,commentsortall".split(',');
  // lcfbiphone,lcfbandroid,lcfundraiserscreate,lcfundraisersfind,lcallinterests,lceditprofile,filterTrendingBox,seemoretrending,fixtrendinglinks,hideTrTopSto,hideTrPolCat,hideTrSciCat,hideTrSpoCat,hideTrEntCat,ASTtoptrends,ASTpolitics,ASTscience,ASTsports,ASTentertainment,lctrendingnews,lc2018election,lclivemap,lcmoments,navbarontop,lcnotes,rooms,chatplaygamebutt,chatstartplanbutt,chatquickcambutt,nonlikedpagepost,chataddfilesbutt,lcfbpnews,lccovid19
  for(var b=0;b<lcoptsarr.length;b++)
	...
	if(fbpoptsobj.colouredbg==1) // neutralise coloured status updates
      fbpfreestyle.textContent+=' ' + individualpostdomquery + ' div[style^="background-color"] *, ' + individualpostdomquery + ' div[style^="background-color"] { color:var(--primary-text) !important; font-weight:normal !important; text-shadow:none !important; background-color:initial !important; text-align:left !important;background:initial !important} ' + individualpostdomquery + ' div[style^="background-color"] img[width="36"][height="36"],'+ individualpostdomquery + ' div[style^="background-color"] img[width="30"][height="30"]{width:22px;height:22px;position:relative;top:3px} ' + individualpostdomquery + ' * {font-size: max(0.92rem, 15px)} ';
    ...
    if(fbpoptsobj.lcallpages==1)
      fbpfreestyle.textContent +=' ' + leftrailcode + ' a[href^="https://www.facebook.com/pages/"],li>div>a[href^="https://www.facebook.com/pages/"], div[data-isanimatedlayout] a[href="/pages/?category=your_pages&ref=bookmarks"], div[role="navigation"] li div a[href^="https://www.facebook.com/pages/"] {display:none !important}';
    ...
    if(fbpoptsobj.lcallgroups==1)
      fbpfreestyle.textContent +=' ' + leftrailcode + ' a[href^="https://www.facebook.com/groups/?ref="],li>div>a[href^="https://www.facebook.com/groups/?ref="], ' + leftrailcode + ' a[href$="/groups/"],li>div>a[href$="/groups/"], div[data-isanimatedlayout] a[href="/groups/"],  div[data-isanimatedlayout] div[aria-label="Groups"], div[role="navigation"] li div a[href^="https://www.facebook.com/groups/"]  {display:none !important}';

    if(fbpoptsobj.lcfriends==1) // Friends
      fbpfreestyle.textContent +=' ' + leftrailcode + ' a[href^="https://www.facebook.com/friends/"],li>div>a[href^="https://www.facebook.com/friends/"], div[role="navigation"] li div a[href^="https://www.facebook.com/friends/"] {display:none !important}';
    ...
    */
    ...
    if(fbpoptsobj.lcreels==1)
      fbpfreestyle.textContent +=' ' + leftrailcode3 + ' div[data-visualcompletion="ignore-dynamic"]>a[href*="/reel/"],li>div>a[href*="/reel/"],div[role="navigation"] li div a[href^="https://www.facebook.com/reel/"] {display:none !important}';
	...
	if(fbpoptsobj.lcwatch==1)
      fbpfreestyle.textContent +=' ' + leftrailcode2 + ' a[href*="/watch/"], a[href="https://www.facebook.com/watch/?ref=mega_menu"], div[role="banner"]+div[data-isanimatedlayout] a[href*="/watch/"], div[role="navigation"] li div[data-visualcompletion="ignore-dynamic"] a[href="https://www.facebook.com/watch/"], div[role="navigation"] li div a[href^="https://www.facebook.com/watch/"] {display:none !important}'; // this bit was hiding datestamps and page names on watch videos so commented out // + leftrailcode3 + ' a[href*="/watch/"],li>div>a[href$="https://www.facebook.com/watch/"], '
   ...
   if(fbpoptsobj.lcmetaai==1)
     fbpfreestyle.textContent +=' div[role="navigation"] a[href^="https://l.facebook.com/l.php?u=https%3A%2F%2Fwww.meta.ai%2F"], div[role="navigation"] a[href^="https://www.meta.ai"] {display:none !important}';

   if(fbpoptsobj.lcaichats==1)
     fbpfreestyle.textContent +=' a[href^="https://aistudio.facebook.com/"] {display:none}';
    ...
    if(fbpoptsobj.rcfriendrequests==1){
	  ...
	  extraTextFilter+=',FacebookFriend Requests';
	}
    ...
    // add filtermatchtext styles
	fbpfreestyle.textContent+=' div.filtermatchtext {color:grey; padding-bottom:4px;padding-top:4px;font-size:13px !important;padding-left:10px; background-color:var(--messenger-card-background)}';

    // add undoBox styles (for autohide)
	fbpfreestyle.textContent+=' .undoBox p {padding-left:30px;color:var(--secondary-text);background-color: var(--secondary-button-background)} .undoBox .p1 {font-weight:800} .undoBox button.undohide-butt {position:relative;left:63%;border-top-right-radius: 6px;border-bottom-right-radius: 6px;border-top-left-radius: 6px;border-bottom-left-radius: 6px;color: var(--secondary-text);  background-color: var(--secondary-button-background);}';
...
//refreshwhitelist(); // no longer needed...
  ...
  if(filterName=="AutoHide"){
    ...
    if(fbpoptsobj.pymk){ // autohide people you may know
      if (tmptextfilter.length)
	    tmptextfilter+=',^X?People you may know,FacebookPeople you may know,People you may knowX';
      else
	    tmptextfilter+='^X?People you may know,FacebookPeople you may know,People you may knowX';
    }
    if(fbpoptsobj.reelsvideo){ // autohide reels
      if (tmptextfilter.length)
	    tmptextfilter+=',^Reels,Reels  ·,Reels and short videos,· Short Video  ·,ReelsCreate,FollowReels,FacebookReels,Short Video  ·';
      else
	    tmptextfilter+='^Reels,Reels  ·,Reels and short videos,· Short Video  ·,ReelsCreate,FollowReels,FacebookReels,Short Video  ·';
    }
  }
  if(filterName=="AllowList"){
	tmptextfilter='';
    tmptextfilter=fbpoptsobj.userappwhitelist;
  }
...
////////////////////////////////
// BEGIN Create Allow List RegEx
///////////////////////////////
if(typeof(fbpoptsobj.userappwhitelist)!='undefined'){
  allowlistRX=buildTextFilterRX("AllowList");
}
//////////////////////////////
// END Create Allow List RegEx
//////////////////////////////

// BEGIN Create Friend Allow List RegEX
//var usernamewhitelistRX=new RegExp(usernamewhitelist);
// END Create Friend Allow List RegEX
...
*/
  ...
  if (fbpoptsobj.condensedchat) {
	// old facebook design (can probably delete this line now)
	...
	fbpboxstyles += ' div[role="complementary"] div[data-visualcompletion="ignore-dynamic"] ul mask,div[role="complementary"] div[data-visualcompletion="ignore-dynamic"] ul circle {display: none } div[role="complementary"] div[data-visualcompletion="ignore-dynamic"] ul svg {height: 16px !important} div[role="complementary"] div[data-visualcompletion="ignore-dynamic"] ul li { height: 16px !important; margin-top: 5px } /*  Group chats */ div[role="complementary"] div[data-visualcompletion="ignore-dynamic"] ul>li>div * {height: 16px !important}';
  }

  if ((typeof fbpoptsobj.rcmetaai !=='undefined') && fbpoptsobj.rcmetaai) {
	fbpboxstyles +=' a[href^="https://www.facebook.com/messages/t/36327,2227039302/"], a[href^="/messages/t/36327,2227039302/"] {display:none !important}';
  }

  if ((typeof fbpoptsobj.startcallbutt !=='undefined') && fbpoptsobj.startcallbutt) {
	fbpboxstyles +=' #ChatTabsPagelet a[data-gt^="{\\"videochat\\""], ul._fl2 div[title^="Start a video chat"],ul._fl2 div[title^="Start a voice call"], #ChatTabsPagelet a[href="Send a voice clip"], div[aria-label="New message"] a[title="Send a voice clip"],div[aria-label="Start a video call"],div[aria-label="Start a voice call"] {display:none !important}';
  }

  /*if ((typeof fbpoptsobj.chataddfilesbutt !=='undefined') && fbpoptsobj.chataddfilesbutt) {
	fbpboxstyles +=' #ChatTabsPagelet form[title="Add files"],div._4rv3._7og6 form[title="Add files"],div[aria-label^="Attach a file"] {display:none !important}';
  }
  */
  ...
  if((typeof fbpoptsobj.fulltimestamps !=='undefined') && fbpoptsobj.fulltimestamps) {  // this code is obsolete, as we cant change timestamps any more
  ...
  if((typeof fbpoptsobj.highlightpublicposts !=='undefined') && fbpoptsobj.highlightpublicposts) {
	fbpboxstyles += ' svg[title^="Shared with Public"] {color:red; width: 1.9em; height: 1.9em; fill: currentColor; vertical-align: middle;}';
  }
  ...
  if((typeof fbpoptsobj.commentbubbles !=='undefined') && fbpoptsobj.commentbubbles) {  // i think this code is obsolete now...
  ...
  // Add FBP Splash Screen (commented out as it doesnt play well with dark mode
 /* fbpboxstyles+=  `
    // Style the splash screen container 
    #splash-screen {
      display: flex;
      flex-direction: column; // Stack F and FBP vertically 
      justify-content: center;
      align-items: center;
      height: 100vh;
      width: 100%;
      background: linear-gradient(135deg, #e7f3ff, #d0e3f0);
      position: absolute;
      top: 0;
      left: 0;
      z-index: 9999;
    }
    // Hide the second SVG (duplicate)
    #splash-screen > div:nth-child(2) {
      display: none !important;
    }
    // Ensure the first SVG (Facebook "F") is visible and styled 
    #splash-screen > div:nth-child(1) {
      display: block !important;
      margin-bottom: 20px; // Space between F and FBP
    }
    // Add "F.B. Purity" below with animation
    #splash-screen::after {
      content: "F.B.P.";
      font-family: Arial, sans-serif;
      font-size: 40px;
      font-weight: bold;
      color: #1877f2; // Light Blue 
      text-shadow: 2px 2px 4px rgba(0, 0, 0, 0.2);
      animation: fadeIn 1s ease-in-out infinite;
    }
    // Fade-in animation for FBP text 
    @keyframes fadeIn {
      0% { opacity: 0.5; }
      50% { opacity: 1; }
      100% { opacity: 0.5; }
  ...
  */
  ...
  if(typeof(fbpoptsobj.friendswhitelistArr)!='undefined') {
	...
	if(typeof (friendswhitelistArr)=='object') {
      for(i in friendswhitelistArr) { // i is the friend id
        //useridwhitelist+= '|' + i; // at this point, useridwhitelist already contains FBPFanPageID and FBPDevPageID and currentuserid 
		usernamewhitelist+='|' + friendswhitelistArr[i];
      }
	}
	//var userFullName=getUserFullName();
	//console.log("userFullName=",userFullName);
	// if userFullName is not null add it to friendwhitelist
	// unfortunately at this point userFullName is null as the required facebook code has not yet loaded. could workaround it but for now just commenting this out
	
	if(debug)
	  //console.log("AFTER WHITELIST IS FINALISED userwhitelist= :  " + useridwhitelist);
      console.log("AFTER WHITELIST IS FINALISED userwhitelist= :  " + usernamewhitelist);
      //useridwhitelistRX=new RegExp(useridwhitelist);
	  usernamewhitelistRX=new RegExp(usernamewhitelist);
  }
    ...
    if(records.length) {
      for(q=0;q<records.length;q++) {
	    ...
	    for(p=0;p<records[q].addedNodes.length;p++) {
          ...
          if((tempnode.nodeName=='LI') ||  (tempnode.nodeName=='UL')) { // dont think this branch is used any more, might have been for filtering the ticker?..
          ...
          //else if(tempnode.nodeName=='DIV') {
		  else if(tempnode.nodeName=='DIV' || tempnode.nodeName.includes("-")) { // fix for newsfeed filtering (detect non standard html tags with dash in name) 11/10/25
			/*if(fbpoptsobj.notificationpopup && tempnode.classList.contains('__fb-light-mode') && /^New notification/.test(tempnode.textContent)){
			  alert('notification popup');
			  window.setTimeout(function(notifnode){
				  return function(){
				    //console.log('running div checker');
					if(notifnode.querySelector('a[href$="ref=notif"]') || /^New notification[A-Za-zÀ-ÖØ-öø-ÿ]/.test(notifnode.textContent)){
				      //notifnode.style.display="none";
					  notifnode.style="display:none !important";
					  alert('notification popup');
			...
			//if(fbpoptsobj.nonags){
				//if a div is inserted with the following text content:
                //"Chats aren’t backed up. Fix now"
                //run the folllowing code
                //document.querySelector([aria-label="Dismiss Your chats aren’t backed up card"]).click();
			...
			/* hiding sponsored on watch and search pages does not currently work, so commenting out to avoid error messages
            ...
            //if(fbpoptsobj.sponsoredbox && tempnode.textContent.length && /Skip0\:00/.test(tempnode.textContent)) {//
            if(fbpoptsobj.sponsoredbox && tempnode.textContent.length && /^Skip$|Skip0\:00/.test(tempnode.textContent)) {
              console.log('Skip button appeared, lets click it');
              if(document.querySelector('div[data-instancekey]>div>div+div>span>span')){ // click skip video ad button
                console.log('found skip button in dom, clicking...');
                document.querySelector('div[data-instancekey]>div>div+div>span>span').click();
              }
              else
                console.log('Didnt find skip button in DOM, they must have changed the dom location of the button');
            }

            if(/tings$/.test(location.href)){ // add clear add prefs butt ( tings is short for /ad_settings )
			  if(/(Interest|Other) categoriesRemoving|Interest CategoriesRemoving/.test(tempnode.textContent))
		        addclearadprefsbutt();
		    }

			//if (tempnode.querySelector('[aria-labelledby][aria-describedby]')) {
			if (tempnode.querySelector(individualpostdomquery)) {
			...
			/* Commented out 11/4/25 facebook stopped blocking links to fbpurity.com a while ago, so commenting this bit out (for now)
			...
			if(fbpoptsobj.sponsoredbox && (tempnode.classList=="") && (typeof tempnode.childNodes !="undefined") && (tempnode.childNodes.length==1) && (tempnode.firstChild.nodeName=="A") && (tempnode.firstChild.textContent.match(adString))){
		      if(debug) {
				tempnode.style="border : green solid 6px"; // IS this causing a bug in marketplace??? (it was)
				console.log("code supposed to hide sponsored box under video nodes (code located in insertednodehandler)?",tempnode.textContent, tempnode.firstChild.textContent, adString);
			  }
			  else
                tempnode.style.display='none';
		  ...
		  //else if (tempnode.nodeName=='SPAN' &&  tempnode.querySelector('[aria-labelledby][aria-describedby]')) { // added april 2025
		  else if (tempnode.nodeName=='SPAN' &&  tempnode.querySelector(individualpostdomquery)) { // added april 2025
			//console.log("this div is going to get cleared...",tempnode.textContent);
			cleartheshizzle(tempnode);
          }
		   ...
		   if(fbpoptsobj.sponsoredbox){ // New Ad Hunter
		    ...
		    if(tempnode.parentNode.querySelector && tempnode.parentNode.querySelector('video') && (tempnode.parentNode.textContent.match(adString))){
			  ...
			  if(tempnode.previousSibling.querySelectorAll('video').length>1){
				...
				//console.log("Video Ad currentTime:", tempnode.previousSibling.querySelectorAll('video')[1].currentTime, "Video Ad Duration:", tempnode.previousSibling.querySelectorAll('video')[1].duration);
				tempnode.previousSibling.querySelectorAll('video')[1].addEventListener('loadedmetadata', () => {
                  //console.log('Metadata loaded, duration:', video.duration);
				  console.log("Video MetaData Loaded: Video Ad currentTime:", tempnode.previousSibling.querySelectorAll('video')[1].currentTime, "Video Ad Duration:", tempnode.previousSibling.querySelectorAll('video')[1].duration);
				});
				//tempnode.previousSibling.querySelectorAll('video')[1].currentTime=tempnode.previousSibling.querySelectorAll('video')[1].duration;
				...
				//document.querySelector('div[data-instancekey]>div>div+div>span>span').click(); // click skip button
			  }
			  if(debug){
			    console.log(tempnode.textContent);
  ...
  function getdtsg() {
	...
	if(document.getElementsByClassName('autoexpand_mode').length) { // i think this code branch is obsolete now.
  ...
  function showhiddendivs() {
	...
	for(var i=0;i<hiddivs.length;i++)
	  if (hiddivs[i].classList.contains("hidden_elem")) {
  ...
  var fbptoggleopts=function(e) {
   ...
   try{
    ...
    if(!document.getElementById('fbpoptsdiv') && (document.getElementById('content')||(document.querySelector(fbpOptsInsertPointQuery))) ) {
	  ...
	  if( (isfirefox || isedge) && (!isgmapi) ){
		//console.log(isfirefox,isgmapi);
		document.getElementById('hovercardsoption').style.display=""; // only show the hide hovercards option for the firefox and edge extension because it still works there (MV2 extensions)
	  }
	...
	// add fbp news notifications option
	getfbpnews(); // commented out 4th April 2025 (we cant currently retrieve the fbp news, so no need for this option to be added for now)
  }

  //------------------------
  // BEGIN News Counter Code
  //------------------------
  var fbpnewsinterval=500; // 90=1.5 hours 250=4 hours 500=8 hours
...
//(Object.prototype.toString.call(value) === "[object PointerEvent]")  
    if(!resetdata && resetdata!='' || (Object.prototype.toString.call(resetdata) === "[object PointerEvent]")) { // .match(/^\[object/)) { // if its from a click in navbar, it gets a mouse event object
	  if(issafari)
	...
	console.log("y.lastEntryDate=",y.lastEntryDate);
    var x={};
	...
	window.setTimeout(function() {
	  ...
	  if(document.getElementById("navItem_page_" + FBPFanPageID)) { // i think this if statement is obsolete as it relates to the old fb design. can probably delete it.
  ...
  function checkforlastnewsupdate(lastnewsupdatedata) {
	...
	if((lastnewsupdatedata=='') || (typeof(lastnewsupdatedata)=='undefined') || (lastnewsupdatedata==null)){
	  ...
	  if(debug)
	    console.log("retrieveddate=" + lastnewsupdatedata,typeof(lastnewsupdatedata));
	  if(typeof lastnewsupdatedata === 'string' && JSON.parse(lastnewsupdatedata))
	    newscounterdata=JSON.parse(lastnewsupdatedata);
	  else
	    newscounterdata=lastnewsupdatedata;
	  if((currtime-newscounterdata.lastCheckTime)/1000/60 >= fbpnewsinterval) { // its time to check the feed again
	    ...
	    if(debug){
	      ...
	      console.log("number of stories=",newscounterdata.numStories);
		}
	    if(newscounterdata.numStories>=1) {
			console.log('we need to show the news notification');
		  ...
		  /*if(document.getElementById("navItem_page_" + FBPFanPageID )) { // i think this if statement is obsolete as it relates to the old facebook design
  ...
  function getfbpnewscount() {
    ...
    if (isValidChromeRuntime()) {
      ...
      GM.xmlHttpRequest({
       method: "GET",
	   url: "https://www.fbpurity.com/fbpnewsupdates.txt",
  ...
  function processFBPNewsPageDate(response) { //, UserId: currentuserid
    ...
    if(responseText.match(/\<date\>.*?\<\/date\>/))
      responseText=responseText.match(/\<date\>(.*?)\<\/date\>/)[1];
    else
      console.log('didnt find date on retrieved page, (no date tag found) maybe they have changed the code...')
	...
	if(responseText.match(/\d+/))
	  lastEntryDate=new Date(responseText).getTime();
	else{
  ...
  function filtermatchmsg(mnode, msg) {
    if(mnode.getElementsByClassName('filtermatchtext').length)
	  ...
	  matchspan.setAttribute('class','filtermatchtext'); // the following styles should be added to a style sheet, not added to every matching div...
  ...
  // the following lines are associated with the following function
  var undobox=document.createElement('div');
  undobox.className='undoBox';
  //undobox.innerHTML="<p style='padding-left:30px;font-weight:800;color: var(--secondary-text);background-color: var(--secondary-button-background);'>Post Hidden</p><p style='padding-left:30px;background-color: var(--secondary-button-background);color: var(--secondary-text);'>FBP's Auto Hide Filter has temporarily hidden this post from view.</p><button style='position:relative;left:63%;border-top-right-radius: 6px;border-bottom-right-radius: 6px;border-top-left-radius: 6px;border-bottom-left-radius: 6px;color: var(--secondary-text);  background-color: var(--secondary-button-background);' class='undohide-butt'>Click to view hidden post</button>";
  undobox.innerHTML="<p class='p1'>Post Hidden</p><p>FBP's Auto Hide Filter has temporarily hidden this post from view.</p><button class='undohide-butt'>Click to view hidden post</button>";
  ...
  function cleartheshizzle(thenode) {
	...
	fbpoptsobj.showfbpvidplayerlink=0; // disable "enable fbp video player" option as it no longer works properly... (should maybe remove the code for it at some point, or try and fix it, so leaving it in for now.
    if(((fbpoptsobj.newsfeed==1) && (document.getElementById(newdesignid))) || (typeof(fbpStyleNormal) === 'undefined') || (window.location.href.match(/facebook\.com(\/messages\/?|\/reel\/|\/photo\/?\?|\/groups\/?|.*\/posts\/|\/notifications\/?)/)!==null) ) // stop newsfeed processing if we are on Messages / Groups or Pages //window.location.href.match(/facebook\.com\/messages\//)
...
//  if(typeof thenode===undefined){ // didnt work because typeof always returns a string. try and remember this!!!
	if(thenode==null){
	  thenode=document;
	  if(debug)
	...
	var newest2022style=document.querySelector(individualpostdomquery);
	...
	if(debug)
	  console.log("DEBUG: thenode=", thenode);
    //storynodes=thenode.querySelectorAll('[aria-labelledby][aria-describedby]');
	storynodes=thenode.querySelectorAll(individualpostdomquery);
	/*
	...
	*/
    ...
    if(!storynodes.length)
	  ...
	  else if(thenode.querySelector(individualpostdomquery)){
		storynodes=[];
		storynodes[0]=thenode;
	  }
	...
	*/
	...
	var svguse,svgusexlink,svgusexlinktext,svguse2,svgusexlink2,svgusexlinktext2,shadowroot;
    ...
    for(i=0;i<storynodeslength;i++) {
	  ...
	  storysaction=storynodes[i].querySelector('h5,h4,h3'); // it seems they use a different heading on the default newsfeed and feeds page (currently h4 on default and h3 on feeds page)
	  ...
	  */
	  ...
	  if(typeof storysaction!='undefined' && storysaction){
		// white list fbp news posts
	    //console.log(storysaction + storysaction.length + storysaction[0].textContent);
	    storysactiontext=storysaction.textContent;
		if(/^Fluff Busting \( FB \) Purity$|^Fluff Busting Purity Development \& Testing$/.test(storysactiontext)){
          console.log("Allow Listing FBP Post!",storynodes[i].textContent);
          storynodes[i].setAttribute('class',storynodes[i].classList.value+ ' fbpnormal fbpWL');
		  filtermatchmsg(storynodes[i],'   FBP : Allow List Match - ' + "FBP NEWS UPDATE");
          continue; // stop filtering and proceed to next node...
        } 
	  }
	  else
		storysactiontext="";
	  ...
	  */
      ...
      if(debug)
  	    console.log("storysactiontext=",storysactiontext); // works on default newsfeed but not on Feeds page...
      ...
      //Add the "zap the ads" menu option
	  ...
	  //BEGIN new Oct 2025 Sponsored Post detection code
	  try {
		//if(debug)console.log('oct 25 code');//debug code
        const objectTagsInNode = storynodes[i].querySelectorAll('object');
		//console.log('num objects=',objectTagsInNode.length);//debug code
        if (objectTagsInNode.length >= 3) {  // >=3 to safely access [2]
          const targetObject = objectTagsInNode[2];  // Third object (0-indexed)
          const innerLink = targetObject.querySelector('a');  // Drill down
		  
		  /*if (innerLink)//debug code
		    console.log(innerLink.textContent,innerLink.textContent.replace(/-/g, '').match(adStringRegEx));
		  else
			console.log('didnt find inner link...');
		  */
		  
          //if (innerLink && innerLink.innerText.match(adStringRegEx)) {
		  if (innerLink && innerLink.textContent.replace(/-/g, '').match(adStringRegEx)) {
            if(debug) {
		      console.log("Found An Object Tag Ad Oct 2025 style!!\n\n" + storynodes[i].textContent);
			  storynodes[i].style.border='2px solid red';
              storynodes[i].style.setProperty('display','block','important');
            } else{
              storynodes[i].style.cssText = 'display:none !important';
          ...
          console.log('Error occurred in new ad checking code (OCT25):', e);
      }
	  //END new Oct 2025 Sponsored Post detection code

	  // BEGIN New June 2025 Sponsored Post detection code
	  try{
		if( document.querySelector(storynodes[i].querySelector('SVG:not([class])')?.querySelector("use")?.getAttribute('xlink:href'))?.parentNode?.textContent?.match(adStringRegEx)){
		  if(debug) {
		    console.log("Found An SVG Ad June 2025 style!!\n\n" + storynodes[i].textContent);
			storynodes[i].style.border='2px solid red';
			storynodes[i].style.setProperty('display','block','important');
		  } else{
			storynodes[i].style.cssText = 'display:none !important';
		 ...
		 console.log('sponsored post detector "2025" fecked up',e);
	  }
	  // END New June 2025 Sponsored Post detection code
	  ...
	  if((storynodes[i]?.querySelectorAll('a')?.length > 2) && ((storynodes[i]?.querySelectorAll('a')[3]?.innerText?.match(adStringRegEx)) ||(storynodes[i]?.querySelectorAll('a')[2]?.innerText?.match(adStringRegEx)))){
		storynodes[i].className+=" fbad";
		if(debug) {
...
//////////////////////
	...
	////////////////////////////////
	// BEGIN Check Friend Allow List
	////////////////////////////////
	  //if(usernamewhitelist && usernamewhitelist.length){
      if (usernamewhitelist?.length && !storynodes[i].querySelector('a[aria-label][href="/groups/discover/"]') && (typeof usernamewhitelistRX !== 'undefined')){ //check we have names in the allow list and make sure its not a suggested groups post
		//console.log(usernamewhitelist);
        tmpmatch=storynodes[i].textContent.match(usernamewhitelistRX);
		if(tmpmatch && tmpmatch.length){
		  //console.log("matchlength= " + tmpmatch.length);
		  tmpmatch=tmpmatch[0];
		  // mark story as allowed / whitelisted as theres a text match
		  if(debug)
		    console.log("Friend Allow List Post! " + tmpmatch,storynodes[i].textContent);
          storynodes[i].setAttribute('class',storynodes[i].classList.value+ ' fbpnormal fbpWL');
          //if(debug)
	      filtermatchmsg(storynodes[i],'   FBP : Friend Allow List Match - ' + tmpmatch);
          continue; // stop filtering and proceed to next node...
		}
	  }
    //////////////////////////////
	// END Check Friend Allow List
	//////////////////////////////	
	
	/////////////////////////
	// BEGIN Check Allow List
	/////////////////////////
	  if(fbpoptsobj.userappwhitelist.length && storynodes[i].textContent.length) {
	    tmpmatch=storynodes[i].textContent.match(allowlistRX);
		if(tmpmatch && tmpmatch.length){
		  //console.log("matchlength= " + tmpmatch.length);
		  tmpmatch=tmpmatch[0];
          storynodes[i].setAttribute('class',storynodes[i].classList.value+' fbpnormal fbpWL');
		  filtermatchmsg(storynodes[i],'   FBP : Allow List Match - ' + tmpmatch);
		  //updateblockedcount();
		  tmpmatch=null;
          continue;
		}
	...
	////////////////////////
	// END  Check Allow List
	////////////////////////
	
      // BEGIN show image classification text when mouse pointer hovers over single image post
	  if(fbpoptsobj.imagetagsonhover) {
		//console.log(storynodes[i].textContent.replace('',''))
		...
		if(imageisclassified.length) {
		  for(var imgindex=0; imgindex<imageisclassified.length; imgindex++) {
		    if(imageisclassified[imgindex].getAttribute('alt') && (imageisclassified[imgindex].getAttribute('alt').length && imageisclassified[imgindex].getAttribute('alt').length>1)){
			  ...
			  //console.log("IMAGETEXT=",imageisclassifiedtext);
			}
			else
	  ...
	  // END show image classification text
	  ...
	  */
	  ...
	  //console.log('"'+storysactiontext+'"' + ' ' + headerTextFilter);
      ...
      if((storysactiontext?.length && headerTextFilter?.length && headertextfilterRX.test(storysactiontext)) || (storynodes[i]?.textContent?.includes("\u00b7") && headertextfilterRX.test(storynodes[i].textContent.split("\u00b7")[0]))) {
		storynodes[i].setAttribute('class',storynodes[i].classList.value+' fbpblockedx');
	...
	if((typeof(fbpoptsobj.autohidefilter)!='undefined') && ((fbpoptsobj.autohidefilter!="")||fbpoptsobj.pymk==1||(fbpoptsobj.reelsvideo==1)) && (storynodes[i].textContent.length || ocrImageText.length)){
	  ...
	  let autohidematch=(ocrImageText.length? storynodes[i].textContent + " " + ocrImageText : storynodes[i].textContent).match(autohidefilterRX);
    ...
    ////////////////////////////////////////////////////////////////////////////////////////////
    // BEGIN TEMP code for hiding blank black/grey reels posts if hide reels option is turned on
    if((typeof fbpoptsobj.reelsvideo!=='undefined') && fbpoptsobj.reelsvideo){
	  if(storynodes[i].querySelector('a[aria-label="Open reel in Reels Viewer"]')){
		storynodes[i].setAttribute('class',storynodes[i].classList.value+' fbpblockedx');
		filtermatchmsg(storynodes[i],'   FBP : Post Type Filter Match - Reels');
		updateblockedcount();
        continue;
	  }
	}
	// END TEMP code for hiding blank reels posts if hide reels option is turned on
	/////////////////////////////////////////////////////////////////////////////////////////////
	  ...
	  if((typeof fbpoptsobj.hideallvideos!=='undefined') && fbpoptsobj.hideallvideos) {
		if(storynodes[i].querySelector('a[rel="theater"][ajaxify*="/videos/"],a[href*="youtube"],a[href*="youtu.be"],a[href*="vimeo.com"],img[src*="vthumb"],video,img._9_m,a[href*=".gifv"],input[type="button"][aria-label="Play video"],div[aria-label="Play video"],img[src^="/images/video/play"],a[aria-label="Open reel in Reels Viewer"],a[aria-label="reel"],div[aria-label="Reels"]')) { //img._9_m = animated gif, a[href*="video.php"],div[data-instancekey]
		  ...
		  console.log('we didnt find a video',storynodes[i].textContent.replace("Facebook",''),storynodes[i].innerHTML,storynodes[i]);
		}*/
	  }
	  ...
	  // NEED TO REFACTOR THIS SECTION OF CODE FOR FINDING APPID ETC
		...
		if(fbpoptsobj.commentwall && (typeof(storynodes[i])!='undefined') && storynodes[i].classList.contains("fbpnormal") && (!location.href.match(/\?sk=groups$/))) {
...
function fixexternallinkredirects() { // for injection (injection no longer works, so can probably delete this function)
  ...
  function unmanglelinks(nodetofix) {
	...
	if(nodetofix.getElementsByTagName) {
      ...
      for(var i=0;i<as.length;i++){
		...
		/*if(hidehovercards){ // data-hovercard attributes no longer exist in the "new" facebook code
...
function unmanglelinks(nodetofix) {
  ...
  if(nodetofix.getElementsByTagName) {
    ...
    for(var i=0;i<as.length;i++) {
	 ...
	 /*if(typeof(fbpoptsobj.hovercards)!='undefined' && fbpoptsobj.hovercards){ //data-hovercard attribute no longer exists so commenting this section out
	  if(as[i].getAttribute && as[i].getAttribute('data-hovercard') && as[i].getAttribute('data-hovercard').match(/\/ajax\//))
		if(as[i].removeAttribute)
		  as[i].removeAttribute('data-hovercard');
	  }*/
...
function monitorSoftNavigations() {
  'use strict';

  // Utility to log messages
  const log = (...args) => console.log('[SoftNavMonitor]', ...args);
  ...
  function checkUrlChange() {
    ...
    if (currentUrl !== lastUrl) {
	  if(debug) // temporarily comment out for testing
        log('Soft navigation detected:', currentUrl);
      onSoftNavigation(currentUrl);
      lastUrl = currentUrl;
    }
  }
  ...
  // --- Centralized Handler ---
  function onSoftNavigation(url) {
	if(debug)
      log('Confirmed soft navigation to:', url);
    if(/\/business\.facebook\.com/.test(location.href)) // cant post messages on hosts other than www. (should check for other hosts, but just checking business for now)
	  return;
    //if(url.match(/\/fluffbustingpurity/) && (!url.match(/\/fluffbustingpurity\/settings/))) { // post message to reset the fbp news counter and initiate faq prompt
	if (/\/fluffbustingpurity/.test(url) && !/\/fluffbustingpurity\/settings/.test(url)) { // post message to reset the fbp news counter and initiate faq prompt
	  //if(arguments[0].match(/facebook\.com\/bookfacepurity/) && (!arguments[0].match(/facebook\.com\/bookfacepurity\/settings/))) {
	  //if(arguments[0].match(FBPFanPageURLRegex) && (!arguments[0].match(RegExp("facebook\.com\/" + FBPFanPageURL + "\/settings")))) {
	...
	//else if(url.match(/\/(whounfriendedmemes|hidepeopleyoumayknow|getridofsuggestedposts|hidetrendingtopics|hidepoliticsfromthenewsfeed|sortbymostrecent|disableautoplay|revertsite)/)){
	else if (/\/(whounfriendedmemes|hidepeopleyoumayknow|getridofsuggestedposts|hidetrendingtopics|hidepoliticsfromthenewsfeed|sortbymostrecent|disableautoplay|revertsite)/.test(url)) { // some of these are no longer with us RIP, thanks facebook!		
	  window.postMessage('INITINVITEALL',"https://www.facebook.com");
	}
	//else if(url.match(/\/$|\/\?sk=/)) {
	else if (/\/$|\/\?sk=/.test(url)) {
	  //^\/$|
	...
	else if(url.match(/\/games\/|\/dialog\/oauth\//)) { // i think the next 3 lines are obsolete, and can probably be removed.
	...
	//else if(url.match(/\/photo(s\/|\.php)/)) { //.*&theater$
	else if (/\/photo(s\/|\.php)/.test(url)) {
	  //console.log("theater mode engaged");
	  window.postMessage( "FBP>TheaterMode", "https://www.facebook.com");
	  //window.setTimeout(addsetbackgroundimagelink,4000);
	}
	//else if(url.match(/\/pages\//)){  //dont think this branch is needed anymore
	else if (/\/pages\//.test(url)) {  //dont think this branch is needed anymore
	  window.postMessage("ADDPAGESFEEDLINK","https://www.facebook.com"); //addPagesFeedLink
	}
	///else if(url.match(/\/(friends|friends_all)/))
	else if (/\/(friends|friends_all)/.test(url)) {
	  window.postMessage("INITDEACTIVATEDFRIENDSLINK","https://www.facebook.com");
	}
	//else if(url.match(/\/settings\?tab=notifications\&section=on_facebook/)) // dont think this branch is needed anymore
	else if (/\/settings\?tab=notifications\§ion=on_facebook/.test(url)) { // dont think this branch is needed anymore
	  window.postMessage("INITFBPNOTILINK","https://www.facebook.com");
	}
	//else if(/\/adpreferences|ad_preferences\/?/.test(url)){
    else if (/\/(adpreferences|ad_preferences|Ad_topics$)\/?/.test(url)) { 
	  //window.setTimeout(function(){alert("hoohoaa");},1000);
	  //window.postMessage('INITADPREFSBUTT',"https://www.facebook.com"); // posting message doesnt work if the subdomain is not www.  (we dont need to post messages any more anyway)
	  window.setTimeout(addclearadprefsbutt,2000);
	}
	else if(/\/marketplace\/?/.test(url)){
...
function addfriendtowhitelist() {
  ...
  deletelink.setAttribute('title',"Remove friend from Allow List");
...
function populatefriendwhitelist() {
  ...
  //<div id="frienddiv-$ID">
  var friendtemplate='<a href="/$ID" target="_blank">$NAME</a><input type="hidden" id="friendinput-$ID" value="$NAME::$ID"><a class="removefriend" style="margin-left:4px;color:red" title="Remove friend from Allow List">X</a>';
	...
	if(typeof(delfriendstore[i])!='undefined'){
	  if(!document.querySelector("#delfriendsalertdiv a[href='/" + i + "/']")){
		if(delfriendstore[i].match(/^\>\>\>/) && fbpoptsobj.namechangealerts)
		  newdfa.addnamechangeline("<div style='margin-bottom:2px;margin-left:5px;'><a target='_blank' href='/" + i +"'><img src='//graph.facebook.com/" + i + "/picture?size=small' height='16' width='16' style='vertical-align:middle'></a> &nbsp;<a style='color:#385898' target='_blank' href='/" + i + "/'><b>" + delfriendstore[i].split(/>>>/)[1] + "</b></a> changed name to: <b>" + delfriendstore[i].split(/>>>/)[2] + "</b></div>");
		else if(fbpoptsobj.deletedfriendalerts && !(delfriendstore[i].match(/^\>\>\>/)))
	      newdfa.addline("<div style='margin-bottom:2px;margin-left:75px;font-weight:bold'><a target='_blank' href='/" + i +"'><img src='//graph.facebook.com/" + i + "/picture?size=small' height='16' width='16' style='vertical-align:middle'></a> &nbsp;<a style='color:#385898' target='_blank' href='/" + i + "/'>" + delfriendstore[i] + "</a></div>");
	  }
	  if(debug)
...
function processfriendsobject(friendsobj){
  ...
  if(document.getElementById('fbpoptsdiv') && document.getElementById('fbpoptsdiv').style.display=='block') {
	...
	if(!document.getElementById("friendwhitelistselect")) {
	  ...
	  friendwhitelistaddbutton.textContent='Add friend to Allow List';
  ...
  for(i in oldfriendstore)  { //oldfriendstore is a global variable
    if(typeof(friendstore[i])=='undefined') {
	  ...
	  if(fbpoptsobj.deletedfriendalerts) {
	    ...
	    newdfa.addline("<div style='margin-bottom:2px;margin-left:75px;font-weight:bold'><a target='_blank' href='/" + i +"'><img src='//graph.facebook.com/" + i + "/picture?size=small' height='16' width='16' style='vertical-align:middle'></a> &nbsp;<a style='color:#385898' target='_blank' href='/" + i + "/'>" + oldfriendstore[i] + "</a></div>"); // ( <a target='_blank' title='Show friendship'  href='https://www.facebook.com/" + currentuserid +"?and=" + i + "'>F</a> )
	  }
	}
	...
	//  deletedfriendstore[i] = oldfriendstore[i];  // we actually need to create a new object for storing name changes (or add a flag to say its not a deleted friend but a name change...
	  if(fbpoptsobj.namechangealerts){
        ...
        newdfa.addnamechangeline("<div style='margin-bottom:2px;margin-left:5px;'><a target='_blank' href='/" + i +"'><img src='//graph.facebook.com/" + i + "/picture?size=small' height='16' width='16' style='vertical-align:middle'></a> &nbsp;<a style='color:#385898' target='_blank' href='/" + i + "/'><b>" + oldfriendstore[i] + "</b></a> changed name to: <b>" + friendstore[i] + "</b></div>");	  
...
function processpagesobject(pagesobj){ // this function doesnt currently do anything, so can probably be removed, or else it needs to be worked on further
  ...
  for(i=0;i<unidentifiedPagePosts.length;i++){
    ...
    //data-hovercard attribute no longer exists, so this line looks obsolete
 ...
 if(!document.getElementById('friendwhitelistta') || (!(document.getElementById('friendwhitelistta').style.display=='block'))) {
  ...
  for(i in oldfriendstore)  {
    if(typeof(friendstore[i])=='undefined') {
	  deletedfriendstore[i] = oldfriendstore[i];
	  newdfa.addline("<div style='margin-bottom:2px;margin-left:75px;font-weight:bold'><a target='_blank' href='/" + i +"'><img src='//graph.facebook.com/" + i + "/picture?size=small' height='16' width='16' style='vertical-align:middle'></a> &nbsp;<a style='color:#385898' target='_blank' href='/" + i + "/'>" + oldfriendstore[i] + "</a></div>");
	}
  }
...
function addfbplinktoleftcol() { // add link to fbp fan page in left column of newsfeed. later on we will add the counter for news items, but perhaps it will work automagically?
  // i think this function doesnt work currently, due to the new facebook design, could try and fix it at some point.
  if(document.getElementById(newdesignid)){
...
function fbpurify() {
  //console.log('fbpurify function called');
  if(debug)
    console.trace();
  ...
  if(document.querySelector(individualpostdomquery))
    cleartheshizzle(document);
...
function fbplistener(event){ // listen for messages
  ...
  */
  if (event.data.match(/JSPAGEUPDATE/)) {
    ...
    // BEGIN NEW in fbp v37.5.8 // catch nodes that have dodged processing
    //if(document.querySelector('div[aria-posinset][aria-labelledby][aria-describedby]:not(.fbpnormal):not(.fbpblockedx):not(.fbad)')){
	if(document.querySelector(individualpostdomquery+':not(.fbpnormal):not(.fbpblockedx):not(.fbad)')){
	  if(debug)
	    //console.log('try and catch this one that sneaked past cleartheshizzle...',document.querySelector('div[aria-posinset][aria-labelledby][aria-describedby]:not(.fbpnormal):not(.fbpblockedx):not(.fbad)').textContent);
	    console.log('try and catch this one that sneaked past cleartheshizzle...',document.querySelector(individualpostdomquery+':not(.fbpnormal):not(.fbpblockedx):not(.fbad)').textContent);
	  //cleartheshizzle(document.querySelector('div[aria-posinset][aria-labelledby][aria-describedby]:not(.fbpnormal):not(.fbpblockedx):not(.fbad)').parentNode);
	  cleartheshizzle(); // previous line was only catching one node, this should catch all evaders
	}
	// END NEW in fbp v37.5.8
	...
	window.setTimeout(addfbplinktoleftcol,3000); // dont think this one works anymore
    ...
    if(fbpoptsobj.sortfriendfeed && location.href.match(/facebook\.com\/?$/))
	  window.location.href="https://www.facebook.com/?filter=friends&sk=h_chr";
    if(fbpoptsobj.sortfavorites && location.href.match(/facebook\.com\/?$/))
	  window.location.href="https://www.facebook.com/?filter=favorites&sk=h_chr";
	window.setTimeout(fixsortorder,3000);
	if(fbpoptsobj.sortmostrecent ||  fbpoptsobj.sortfriendfeed || fbpoptsobj.sortfavorites) {
	...
	if(fbpoptsobj.sponsoredbox && !/\/marketplace\/?/.test(location.href)){ // disconnect the marketplace observer
	...
	if(!fbpoptsobj.rcstories) {
	  window.setTimeout(addHideStoriesButt,4000);
	  window.setTimeout(addHideStoriesButt,8000);
	}
  }
...
function hiderightcolumnsection(rcnode){ 
  ...
  var pnode=rcnode;
  while(pnode.classList.length!=0) { // was 1 but facebook changed their code, hopefully its not a glitch... if its a glitch and they revert we need to change back to 1)
  ...
  if(debug){
	console.log('hiding right column box');
    pnode.style.border='3px cyan solid';
  }
  else
...
function clickseemore(section){
  ...
  /*
console.log("Running See More clicker for section:", section);
var matchCount = 0;
var x = document.querySelectorAll('ul+div');

for (var i = 0; i < x.length; i++) {
  var text = x[i].textContent.trim();
  if (/^see more$/i.test(text)) {
    console.log(`Match #${matchCount} at index ${i}`);

    if (section === "explore" && matchCount === 0) {
      console.log("→ Clicking See More (Explore)");
      x[i].querySelector('span')?.click();
      break;
    }
    if (section === "shortcuts" && matchCount === 1) {
      console.log("→ Clicking See More (Shortcuts)");
      x[i].querySelector('span')?.click();
      break;
    }
    matchCount++;
...
// fix top stories link for new design
  ...
  if(!document.getElementById('newtopstorieslinkdiv')){
	...
	var newpipeseperator= document.createTextNode(" | ");
	newtopstorieslinkdiv.appendChild(newpipeseperator);
	
	var newcreatepostlink=document.createElement('a');
	newcreatepostlink.textContent="Create Post";
	newcreatepostlink.title="Open the hidden menu where the Create Post button lives";
    newcreatepostlink.addEventListener('click',function(e) {e.preventDefault();document.querySelector('div[aria-label="Menu"]').click();},false);
	newtopstorieslinkdiv.appendChild(newcreatepostlink);
...
function doubleCheckFirstPost() { 
    // the first post is sometimes being hidden as an ad, when it actually isn't. This function will unhide it and rescan it to make sure its filtered correctly
	if(debug)
      console.log('Running doubleCheckFirstPost');
    var firstpost=document.querySelector('div[aria-posinset="1"].fbad');
	if(!firstpost)
	  return;
    firstpost.classList.remove('fbad');
    firstpost.style="";
    cleartheshizzle(firstpost.parentNode);
}

function addnewsfeedmenufeeds(){ // old code for old design, no longer needed, can delete...
    if(debug)
...
function addclearadprefsbutt() {
  ...
  else if (document.querySelector('[aria-label^="Search ad topics"]')) { // its the topics page aria-label="Search all topics"
	//alert('its the topics page');
	clearadprefsbutt.style.position="relative";
	clearadprefsbutt.style.left="-10px";
	apbinsertpoint=document.querySelector('[aria-label^="Search ad topics"]');
	apbinsertpoint.parentNode.insertBefore(clearadprefsbutt,apbinsertpoint.nextSibling)
  }
  else if (document.body.textContent.match(/Other categories|Other CategoriesRemoving/) && document.querySelectorAll('div[aria-label="Remove"]').length){ // clear the categories on the categories popup
...
function clearadprefs() {
  if (location.href.match(/\/adpreferences\/(advertisers|\?entry_product=)/)) {
	...
	if(document.querySelector('div[role="button"]>div>div>div>span.x1lliihq'))
      if(document.querySelector('div[role="button"]>div>div>div>span.x1lliihq').textContent.match('View and manage topics')){
	    ...
	    if(document.querySelector('div[role="alert"]')?.textContent.match(/^We've updated some topic/)) // close pointless alert/dialog
          document.querySelector('div[role="alert"]').nextSibling.firstChild.click();
        clearadtopics();
	    return;
	  }
  ...
  /////// END AD TOPICS FUNC
}else if(document.body.textContent.match(/Other categoriesRemoving/)||document.querySelector('div[role="button"][aria-label="See All Interests"]')||document.querySelector('div[role="dialog"]').textContent?.match(/^Interest Categories/)) { 
   // Clearing Interests (runs on this page: https://www.facebook.com/adpreferences/?section=interests )
   //if(debug)
   ...
   if(document.querySelector('div[role="button"][aria-label="See All Interests"]')) // new button
     document.querySelector('div[role="button"][aria-label="See All Interests"]').click();
   window.setTimeout( function(){
...
function clearadtopics(){
  ...
  if(debug)
    console.log('Clearing ad topics');
 
 // display all topics and dismiss useless dialog box
 if(document.querySelector('div[role="button"]>div>div>div>span.x1lliihq'))
   if(document.querySelector('div[role="button"]>div>div>div>span.x1lliihq').textContent.match('View and manage topics')){
    document.querySelector('div[role="button"]>div>div>div>span.x1lliihq').click();
	if(document.querySelector('div[role="alert"]')?.textContent.match(/^We've updated some topic/)) // close pointless alert/dialog
  ...
  // Lets clear all ad topics
  window.setTimeout(function(){
    function clickButton(buttonId) { // maybe we should wait for the button title to be displayed (rather than use a timeout)
      return new Promise((resolve, reject) => {
        const button = buttonId;
        if (button) {
		  console.log("Topic to Click:", buttonId.textContent);
          button.click();
    ...
    function clickOptOutButton() {
      return new Promise((resolve, reject) => {
        var optOutButton = document.querySelector('input[value="OPT_OUT"][aria-checked="false"]');
		// the following commented out line resets to no preference for testing purposes
		//var optOutButton = document.querySelector('input[value="NO_PREFERENCE"][aria-checked="false"]');
        if(optOutButton) {
          optOutButton.click();
          resolve();
        } else {
            resolve('opt out button found but already clicked.');
          //console.log('opt out button not found')
          //resolve();
        }
      });
    }
    ...
    function clickBackButton() {
      return new Promise((resolve, reject) => {
        var backButton = document.querySelector('div[aria-hidden="false"] div[aria-label="Back"][role="button"]:not(.arse)');
          document.querySelector('div[aria-hidden="false"] div[aria-label="Back"][role="button"]:not(.arse)').classList+=" arse";
        if (backButton) {
          backButton.click();
          resolve();
        } else {
    ...
    //const buttonIds = document.querySelectorAll('div[role="dialog"] div[role="button"]:not([aria-label="Close"]):not([aria-label="Back"])');
      document.querySelectorAll('div[aria-label="Back"][role="button"]')[0].classList+=" arse"
      //document.querySelectorAll('div[aria-label="Back"][role="button"]')[1].classList+=" arse"
    var buttonIds = document.querySelectorAll('div[role="dialog"] div[aria-hidden="false"]  div[role="listitem"] div[role="button"]:not([aria-label="Close"]):not([aria-label="Back"])');
	//const buttonNames = document.querySelectorAll('div[role="dialog"] div[aria-hidden="false"]  div[role="listitem"] div[role="button"]:not([aria-label="Close"]):not([aria-label="Back"])');

    async function clickAllButtons() {
      //for (const buttonId of buttonIds) {
	  for (i=0;i<buttonIds.length;i++) {
        try {
		  if(buttonIds.length!=document.querySelectorAll('div[role="dialog"] div[aria-hidden="false"]  div[role="listitem"] div[role="button"]:not([aria-label="Close"]):not([aria-label="Back"])').length){
		    buttonIds=document.querySelectorAll('div[role="dialog"] div[aria-hidden="false"]  div[role="listitem"] div[role="button"]:not([aria-label="Close"]):not([aria-label="Back"])');
		    --i;
	      }
		  const buttonId=document.querySelectorAll('div[role="dialog"] div[aria-hidden="false"]  div[role="listitem"] div[role="button"]:not([aria-label="Close"]):not([aria-label="Back"])')[i];
          if(/Review topic choices/.test(buttonId.textContent)){
            continue;
          }
		  console.log("i=",i);
          await clickButton(buttonId);
          console.log(`Clicked Topic button.`);
          await new Promise(resolve => setTimeout(resolve, 2500));// was 5000
          await clickOptOutButton();
          console.log(`Clicked Opt-out button.`);
          await clickBackButton();
          console.log(`Clicked Back button.`);
		  //if(i==(buttonIds.length-1)) // they have started removing rows after show less is clicked, so we need to keep checking the length of the array so we know when to quit
	      if(i==document.querySelectorAll('div[role="dialog"] div[aria-hidden="false"]  div[role="listitem"] div[role="button"]:not([aria-label="Close"]):not([aria-label="Back"])').length-1){
		    alert('F.B. Purity Finished Clearing Ad Topics!!');
			break;
		  }
          await new Promise(resolve => setTimeout(resolve, 6500)); // was 5000
        } catch (error) {
            console.error(error);
        }
      }
...
function initInviteAllButton3(){
  ...
  if(document.querySelector('[aria-label="Profile settings see more options"][role="button"]'))
    document.querySelector('[aria-label="Profile settings see more options"][role="button"]').click()
...
function initInviteAllToLikePage() {
  ...
  initInviteLink.setAttribute('title',"Invite All Your Friends");
  initInviteLink.style.cursor = "pointer";
  initInviteLink.style.marginTop = "5px";


  var invitelinkinsertpoint;
  /*if(document.querySelector('div[aria-orientation="horizontal"][role="tablist"]') && document.querySelector('div[aria-orientation="horizontal"][role="tablist"]').querySelector('div[aria-controls]')){
	invitelinkinsertpoint=document.querySelector('div[aria-orientation="horizontal"][role="tablist"]') && document.querySelector('div[aria-orientation="horizontal"][role="tablist"]').querySelector('div[aria-controls]');
  }
  else { console.log('second bra')*/
  invitelinkinsertpoint=document.querySelector('div[aria-label="Search"]');
  /*}*/

  if(invitelinkinsertpoint && typeof(invitelinkinsertpoint.parentNode)!=="undefined"){
    /*invitelinkinsertpoint.parentNode.insertBefore(shadowhostdiv,invitelinkinsertpoint.nextSibling);
	shadowRoot.appendChild(initInviteLink);
	*/
  ...
  if(initInviteLink) {
  //if(document.getElementById('shadowy'))
...
function initdeactivatedfriendslink (){
  ...
  if(!window.location.href.match(/facebook\.com\/.*\/friends|friends_all|facebook\.com\/friends\/?$/))
    return;
  // old design
  if(!document.querySelector('div[aria-label="Edit profile"],a[aria-label="Edit profile"]')){ //("ssrb_root_start")&& document.querySelector("#pagelet_timeline_main_column").getAttribute('data-gt').match(currentuserid)) {
...
function showDeactivatedFriendsList() {
  ...
  var x=setInterval(function(){
    if(!seenall){ //document.querySelector('div[data-pagelet="ProfileAppSection_1"]')
	  ...
	  allfriends=document.querySelectorAll('img[height="80"][width="80"]:not([processed]),img[height="36"][width="36"]:not([processed])');
	  ...
	  for(i=0;i<allfriends.length;i++)
		if( allfriends[i].parentNode.nodeName!="A"){
		  ...
		  allfriends[i].setAttribute('processed',"1");
          if(allfriends[i].parentNode.parentNode.nodeName=="DIV"){ // we will hide div 3 levels up from img tag
		    //allfriends[i].parentNode.parentNode.parentNode.style.border="red solid 2px";
            allfriends[i].parentNode.parentNode.parentNode.parentNode.removeChild(allfriends[i].parentNode.parentNode.parentNode);
          }
          else {// its probably a span, // we will hide div 4 levels up from img tag
            //allfriends[i].parentNode.parentNode.parentNode.parentNode.style.border="red solid 2px";
		    allfriends[i].parentNode.parentNode.parentNode.parentNode.parentNode.removeChild(allfriends[i].parentNode.parentNode.parentNode.parentNode);
          }
		}
	  ...
	  try{
		...
		console.log("the show deactivated friends link isnt there so we cant modify it...");
	  };
...
// we should check to see if fbpurify is already running...
  ...
  var waitforbody=window.setInterval(function(){
    ...
    // if the mobile version of the site is loaded display message and exit
    // this code seems to freeze the browser occasionally... >???
	...
	window.setTimeout(doubleCheckFirstPost,3000); // make sure first post gets filtered correctly
	
	window.setTimeout(enableCommentSortModeIfNeeded,2000);
	...
	*/
    ...
    window.setTimeout(determineFacebookLanguage,2000); // moved code into its own function, so i could add a delay
	//window.setTimeout(determineFacebookLanguage,4000); // run again just in case first time wasnt sucessful
      ...
      if(fbpoptsobj.sponsoredbox){
	    //window.setTimeout(mopupstragglerads,0); // this line seems to be causing a problem (commented out 22 March 2025
	  ...
	  window.setTimeout(catchStragglerPosts,1000); // catch posts that have got past FBP's filters and have not been processed at all. (might need to run this on a longer timer to catch stray posts that dont get caught for whatever reason)
	  ...
	  if(document.location.href.match(/facebook\.com\/(fluffbustingpurity|whounfriendedmemes|hidepeopleyoumayknow|getridofsuggestedposts|hidetrendingtopics|hidepoliticsfromthenewsfeed|sortbymostrecent|disableautoplay|revertsite)/)) {
        //window.setTimeout(initInviteAllToLikePage,4000);
	    window.setTimeout(initInviteAllToLikePage,8000);
	  }
  ...
  //if(document.querySelector('div[aria-posinset]'))   {
  if(document.querySelector(individualpostdomquery))   {
    fbpurify();
...
function determineFacebookLanguage() {
	// moved code to separate function, with 2 second delay via setTimeout, as otherwise the langcode wasnt being found
	try{
	  var langCodeMatch=String(document.body && document.body.className).match(/Locale_([a-z]{2}_[A-Z]{2})/);
	  if(langCodeMatch){
		...
		console.log("FBP: Determining langCode Failed",e);
		//alert(document.querySelector('html')?.getAttribute('lang'));
	    langCode="en_US";
		
		///DEUG STUFF
		// Select all <link> elements with rel="preload"
		/*const preloadLinks = document.querySelectorAll('link[rel="preload"]');
		// Iterate over the NodeList and print each href
	  ...
	  if(debug)
	    console.log("adString = " + adString , "adStringLength = " + adStringLength , " Secondary Ad String (adString2) = ",adString2 );

      // hide right column sponsored ads
	  if(fbpoptsobj.sponsoredbox)
	    ...
	    //console.log(' a[aria-label="' + AD_LANG_MAP[langCode][1] + '"], ' + 'span[aria-label="' + AD_LANG_MAP[langCode][1] + '"]' + ' {display:none !important}');
		  ...
		  console.log('Using default (right column) "Advertiser link" code');
		  document.getElementById('fbpboxstyler').textContent += ' a[aria-label="Advertiser link"], span[aria-label="Advertiser link"], a[aria-label="Advertiser"]{display:none !important}';  // use the default
...
function mopupstragglerads() { // i think this function is now obsolete and can probably be removed
	if(debug)
...
//BEGIN NEW MARKETPLACE OBSERVER code
function startMarketPlaceObserver() { 
  // hides the ads in the right column of marketplace classified ad pages
  const targetNode = document.documentElement;
  const config = { childList: true, subtree: true };

  if (typeof marketPlaceObserver === "object") return;

  const callback = function() {
    // Always query all unprocessed __fb-light-mode divs
    document.querySelectorAll('div.__fb-light-mode:not(.processed)').forEach(div => {
      // Mark it processed
      div.classList.add('processed');

      // Delay slightly to let React fill the content
      setTimeout(() => {
        if (!div.textContent.includes(adString2)) return;

        const targetSpan = div.querySelector('[id]');
        if (!targetSpan) return;

        const el = document.querySelector(`a[aria-labelledby="${targetSpan.id}"]`);
        if (!el) return;
    ...
    // Call the category page Sponsored Posts handler
    hidemarketplaceads();
  };
...
//END NEW MARKETPLACE OBSERVER CODE

// BEGIN Hide Marketplace ads
function hidemarketplaceads(){
  if(debug)
    console.log("hidemarketplaceads()");
  if(/^https\:\/\/www\.facebook\.com\/marketplace\/you\/selling/.test(location.href)){ // dont hide sellers listings
	console.log('exiting hidemarketplaceads() because we are on the selling page')
    return;
  }
  //var x=document.querySelectorAll('[aria-label="Open Menu"]:not(.processed),[aria-label="Open menu"]:not(.processed)');
  //var x=document.querySelectorAll('[aria-label*=" Menu"]:not(.processed),[aria-label*=" menu"]:not(.processed)');
  document.querySelector('div[role="banner"] [aria-label^="Men"]:not(.processed)')?.classList?.add('processed'); // make sure we dont hide the 3x3 grid button
  
  var x=document.querySelectorAll('[aria-label*="Men"]:not(.processed):not([aria-label^="Open Attachment"]):not([aria-label*="Mens"]),[aria-label*="men"]:not(.processed):not([aria-label^="Open Attachment"]):not([aria-label*="mens"])'); // (some languages have the u in menu with an accent over it... matching just men should make the code work for a couple more languages (hopefully there will be no side effects)
  var test,i,j;
  for(i=0;i<x.length;i++) {
    test=x[i].parentNode;
    x[i].classList.add('processed');
    for (j=0;j<10;j++)
      if(test.nodeName=="SPAN") {  // also check if we are on the /selling page, if so dont hide ads
		if(debug)
...
function handleIntersect(entries, observer) {
  entries.forEach(function(entry) {
	...
	window.setTimeout(adcatcher,100,entry.target,adString,2); // the adcatcher function is running the enigma decoder, which is possibly not needed any more
    ...
    if(fbpoptsobj.threedphoto) { // this functionality doesnt seem to work
	...
	// END Catch 3D photo posts on the second pass
	 ...
	 // BEGIN Check for videos on second pass
	 if(fbpoptsobj.hideallvideos) {
		 if((entry.target?.querySelector('video'))){
		   console.log('video found on second pass');
		   entry.target.classList.remove('fbpnormal');
           entry.target.classList.add('fbpblockedx');
           filtermatchmsg(entry.target,'   FBP : Post Filter Match - ' + " Hide All Videos ");
		   updateblockedcount();
		 }
	 }
	 // END Check for videos on second pass
...
function catchStragglerPosts() { // patch for bug where single post at top of feed didnt get scanned
	...
	//var feedposts=document.querySelectorAll('div[aria-posinset][aria-describedby][aria-labelledby]:not(.fbpnormal):not(.fbpblocked):not(.fbpblockedx):not(.fbad)');
	var feedposts=document.querySelectorAll(individualpostdomquery+':not(.fbpnormal):not(.fbpblocked):not(.fbpblockedx):not(.fbad)');
...
function addHideStoriesButt() {
  ...
  if((!(document.querySelector("#stories_pagelet_rhc,#stories_pagelet_below_composer,div[data-pagelet='Stories'],div[aria-label='Stories'][role='region']"))) || (document.getElementById('shadowx')) || (document.getElementById('hideStoriesButt')))
	return;
  ...
  hideStoriesButt.style.cursor = "pointer";

  hideStoriesButt.setAttribute('title','FBP: Hide Stories Section');
  hideStoriesButt.addEventListener('click',function(){if(!confirm("Do you want to hide the 'Stories' section?\n\nPlease note: If you click 'OK' the page will reload and the whole 'Stories' section will be gone. If afterwards, at some point you would like to restore it again, you can untick the Hide Stories setting on the FBP options screen under the 'More Options' heading.")) return; fbpoptsobj.rcstories=1;
	fbpsavevalue('fbpoptsjson-' + currentuserid, JSON.stringify(fbpoptsobj));
  ...
  if(document.querySelector("#stories_pagelet_rhc a[data-hover='tooltip']"))
	...
	const shadowhostdiv=document.createElement('div');
    shadowhostdiv.id="shadowx";
	shadowhostdiv.style.display="inline";
    const shadowRoot = shadowhostdiv.attachShadow({ mode: 'open' });
  
    hideStoriesButt.style.position='relative';
    hideStoriesButt.style.top='-17px';
	hideStoriesButt.style.zIndex='999';
	hideStoriesButt.style.cssFloat="right";
    hideStoriesButt.style.color='var(--primary-text)';
	...
	document.querySelector('div[aria-label="Stories"][role="region"]').parentNode.insertBefore(shadowhostdiv,document.querySelector('div[aria-label="Stories"][role="region"]'));
	shadowRoot.appendChild(hideStoriesButt);
    console.log('[Shadowx] Link added successfully');
  }
  else
	window.setTimeout(function(){
	  if(document.querySelector("#stories_pagelet_rhc a[data-hover='tooltip']"))
...
// BEIGN Newsfeed Pause button Code
/*
function togglePauseNewsfeed () {
	alert("toggle pausing newsfeed");
  var allnewsposts=document.querySelectorAll('div.x1lliihq');
  if(document.querySelectorAll('div.x1lliihq').length && document.querySelectorAll('div.x1lliihq').length > 1) {
	if(document.querySelectorAll('div.x1lliihq')[1].parentNode.lastChild.previousSibling.previousSibling.style.display=="" || document.querySelectorAll('div.x1lliihq')[1].parentNode.lastChild.previousSibling.previousSibling.style.display=="block"){
      document.querySelectorAll('div.x1lliihq')[1].parentNode.lastChild.previousSibling.previousSibling.style.display="none"
	  document.getElementById('pauselink').textContent="Play";
	}
    else {
	  document.querySelectorAll('div.x1lliihq')[1].parentNode.lastChild.previousSibling.previousSibling.style.display="block";
	  document.getElementById('pauselink').textContent="Pause";
	}
  }
}

var x=document.querySelectorAll('a[href="/"]')[1]; 
var pauselink=document.createElement('a');
pauselink.textContent="Pause";
pauselink.title="Pause the Newsfeed";
pauselink.style="font-weight:500;font-size:30px";
pauselink.id="pauselink";
x.parentNode.insertBefore(pauselink,x);
document.getElementById('pauselink').addEventListener('click',togglePauseNewsfeed,false);
*/
// END   Newsfeed Pause button code
...
// BEGIN COMMENT SORTING CODE
var fbpcommentsortmode;
function enableCommentSortModeIfNeeded() {
  fbpcommentsortmode =
    fbpoptsobj?.commentsortall == 1 ? "All comments" :
    fbpoptsobj?.commentsortnewest == 1 ? "Newest" : "";
  if(debug)
    console.log("fbpcommentsortmode=",fbpcommentsortmode);
  if (fbpcommentsortmode) {
	if(debug)
      console.log('running forceallcomments');
    // Run once when page loads
    forceAllComments();

    // Re-run if the page changes (Facebook is a SPA)
    const fbpCommentSortObserver = new MutationObserver(() => {
      forceAllComments();
    });
    fbpCommentSortObserver.observe(document.body, { childList: true, subtree: true });
  }
}

function forceAllComments() {
  // Find the sort button/dropdown
  const sortButtons = document.querySelectorAll('div[aria-haspopup="menu"][role="button"][aria-expanded="false"]:not([data-fbp-sorted="1"])');
  sortButtons.forEach(btn => {
    // If it's not already "All comments", click it
    if (!/Most relevant/i.test(btn.innerText || btn.textContent || "")) return;

      btn.click();
      btn.dataset.fbpSorted = "1";

      // Wait for menu to open, then click "All comments"
      const observer = new MutationObserver(() => {
        //const menuItem = Array.from(document.querySelectorAll('div[role="menuitem"]'))
        //  .find(el => el.textContent.includes(fbpcommentsortmode));
		const menuItems = Array.from(document.querySelectorAll('div[role="menuitem"]'));
        const menuItem = menuItems.find(el => el.textContent.includes(fbpcommentsortmode));
        ...
        if (menuItem) {
          ...
          // Menu opened, but the required item doesn't exist -> close the menu
          btn.click(); // toggle the button to close the menu
          observer.disconnect();
        }
      });
...
function doc_keyUp(e) {
  ...
  */
  ...
  case 122: //z
    ...
    break;
  /*case 112: //p/
    console.log('P key pressed, try and toggle pausing the newsfeed from loading new posts');
    togglePauseNewsfeed();
	break;
