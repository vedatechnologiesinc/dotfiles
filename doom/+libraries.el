...
(defvar *my-org-babel-languages*
  '(lisp emacs-lisp shell dot python ditaa plantuml ly)
  "List of Org Babel languages to load")

(defun my-org-babel-languages ()
  "Return a list that the variable ORG-BABEL-LOAD-LANGUAGES can use."
  (cl-loop for language
           in *my-org-babel-languages*
           collect `(,language . t)))
...
(after! org
  ...
  (setq org-directory "~/org/"
        ...
        org-todo-keywords-for-agenda '("TODO" "DONE")
        org-html-head (concat "<style>" (file-string (my-doom-path "org.css")) "</style>")
        org-babel-load-languages (my-org-babel-languages)
...
(after! org-roam
  ...
  (cl-flet ((fn (list)
              (cl-loop for name
                       in list
                       collect (let ((key (char-to-string (aref name 0)))
                                     (template "#+title: ${title}\n#+date: <%<%Y-%m-%d %H:%M:%S %z>>\n"))
                                 `(,key ,name plain "%?"
                                   :target (file+head ,(cat name "/${slug}.org") ,template)
                                   :unnarrowed t)))))
    (setq org-roam-directory "~/org/"
          org-roam-db-location "~/org/roam.db"
          org-roam-dailies-directory "~/org/sivykarni/"
          org-roam-capture-templates (fn '("kampu" "vti" "lisp" "skullgirls" ))))
...
(after! sly
  ...
  (pushnew 'sly-repl-ansi-color sly-contribs)
  ...
  (pushnew 'sly-indentation sly-contribs)
  (setq lisp-indent-function 'sly-common-lisp-indent-function)

  ;; (pushew 'sly-stickers sly-contribs)
  ;; (pushew 'sly-fancy-inspector sly-contribs)
  ;; (pushew 'sly-autodoc sly-contribs)

  (def my-sly-mrepl-mode-hook ()
    (smartparens-mode -1))
  (add-hook! sly-mrepl-mode #'my-sly-mrepl-mode-hook)
...
(after! smartparens
  ...
  (add-hook! org-mode #'turn-on-smartparens-strict-mode)
...
(after! popup
  (set-popup-rules!
    '(("^ \\*" :slot -1)
      ...
      ("^\\*sly-mrepl\*" :slot -1 :select t :quit nil :size 0.33 :side bottom)
...
(after! dirvish
  ...
  (transient-define-prefix dirvish-ls-switches-menu ()
    ...
    :init-value
    (λ (o) (oset o value (split-string (or dired-actual-switches ""))))
    [:description
     (λ ()
       (format "%s\n%s %s\n%s %s"
...
(after! ox-pandoc
  (setq org-pandoc-options-for-docx
        `((reference-doc . ,(ef "~/.doom.d/reference.docx")))))
