...
def git {
  ...
  if [[ $# -eq 0 ]]; then
    ...
    case "${op}" in
      ...
      aul|u) "${self}" aum "$(git log -1 --pretty=%B)" ;;
      ...
      zd) "${git}" log -G "${@}" --source --all --pretty=format:"%ci" ;;

      zct) "${self}" h --pretty=format:"%ai" `${self} zc "${@}" | tail -n 1` ;;
      zch) "${self}" h --pretty=format:"%ai" `${self} zc "${@}" | head -n 1` ;;

      zdt) "${self}" h --pretty=format:"%ai" `${self} zd "${@}" | tail -n 1` ;;
      zdh) "${self}" h --pretty=format:"%ai" `${self} zd "${@}" | head -n 1` ;;
...
def gits {
  echo "I thought what I’d do was, I’d pretend I was one of those deaf-mutes."
}
...
def_getter yt y.txt $YOUTUBE -c -i --add-metadata --merge-output-format mkv --write-auto-sub
...
def haskell! {
  if [[ $# -eq 1 ]]; then
    vix r nixpkgs\#stack -- new "$1" && \
    cd "$1" && \
    rz -v ~/src/haskell/base/{{flake,shells,haskell}.nix,flake.lock,.env{,rc},fourmolu.yaml} .
    direnv allow
  else
    echo "No project name specified. Exiting."
    return 1
  fi
}
...
def docx2org {
  pandoc -f docx -t org -o "$(basename $1 .docx).org" "${1}"
}

def scaps {
  local start=${1[0,1]:u}
  local rest=$(smallcaps ${1[2,-1]})
  echo -n "${start}${rest}" | c
}
...
funs=(
  ...
  # the backslash is for extended_glob
  ...
  org! "+ dorg sc -dmS org watchexec make"
  pm! "+ d ~ sc -dmS pm pm google.com"
  ...
  g "rg --color auto"
  gi "g -i"
  less "bat --plain --paging always --theme gruvbox-dark"
  v "less"
  va "v -A"
  rip "=rip --graveyard ~/.local/share/Trash"
  tree! "tree -L 2"
  dns! "sudo dscacheutil -flushcache; sudo killall -HUP mDNSResponder"
  audio! "sudo killall coreaudiod bluetoothd bluetoothaudiod"
  gim "open -a Gimp"

  # lisp
  sbcl! "rl sbcl"
  lispworks! "rl lispworks"
); def_funs
